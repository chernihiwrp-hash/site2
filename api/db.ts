...
const SINGLE_ROW_REQUIRED = new Set([
  ...
  "bans","house_confiscations","mayor_election",
]);
...
export default async function handler(req: any, res: any) {
  ...
  if ((op === "delete" || op === "update") && (!match || !Object.keys(match).length)) {
    return deny(400, `match is required for "${op}"`);
  }
  if (op === "upsert" && (!match || !Object.keys(match).length) && !onConflict) {
    return deny(400, `match or onConflict is required for "upsert"`);
  }
  if (match) {
