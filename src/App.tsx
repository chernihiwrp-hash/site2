...
import Casino from "./pages/Casino";aimport { useState, useEffect } from "react";
import { QueryClient, QueryClientProvider } from "@tanstack/react-query";
import { BrowserRouter, Route, Routes } from "react-router-dom";
import { Toaster as Sonner } from "@/components/ui/sonner";
import { TooltipProvider } from "@/components/ui/tooltip";
...
import { dbVerify, dbCheckUser, dbCheckTelegram, dbRegister } from "./lib/db";
...
const LoginModal = ({ savedNick, onDone, onReset }: { savedNick: string; onDone: () => void; onReset: () => void }) => {
  ...
  const handleLogin = async () => {
    ...
    setLoading(true);
    setError("");
    const { data } = await dbVerify(savedNick, password);
...
const RegisterModal = ({ onDone }: { onDone: (nick: string) => void }) => {
  ...
  const handleRegister = async () => {
    ...
    if (tgUser?.id) {
      const { data: tgBound } = await dbCheckTelegram(String(tgUser.id));
    ...
    if (nick.trim().toLowerCase() === reserved) {
      const { data: existing } = await dbCheckUser("T1kron1x");
      if (existing?.exists && tgUser && String(tgUser.id) !== String(existing.telegram_id)) {
    ...
    const { data: existingNick } = await dbCheckUser(nick.trim());
    if (existingNick?.exists && tgUser && String(tgUser.id) !== String(existingNick.telegram_id)) {
      setError("Цей нік вже зайнятий!");
      setLoading(false);
      return;
    }

    // Зберігаємо через сервер (SERVICE_ROLE_KEY, обхід RLS)
    const { error: dbError } = await dbRegister({
      nick: nick.trim(),
      password,
