...
/* ──────────────────────────────────────────────────────────────────
   Battle Pass v3 — Premium Visual Redesign 2026 (Enhanced Glow & Speed)
   ────────────────────────────────────────────────────────────────── */
...
const RARITY_CONFIG: Record<Rarity, {
  ...
  mythic: { 
    label: "Міфічний",    
    color: "#ff0055",   
    glow: "rgba(255,0,85,1.0)",    
    rgb: "255,0,85",    
    border: "rgba(255,0,85,0.75)",     
    icon: Flame,
...
const SlotCard = ({
  ...
  // Динамическое ошеломительное неоновое свечение для Legendary и Mythic карт (выходит за рамки)
  const getGlowStyles = () => {
    if (locked) return { boxShadow: "none" };
    
    let baseShadow = `0 10px 25px -5px rgba(0,0,0,0.6), inset 0 1px 1px rgba(255,255,255,0.12)`;
    if (isAnimated) {
      // Экстремальный каскад внешних теней, пробивающийся сквозь темноту на 15-30px
      baseShadow += `, 0 0 20px -2px ${cfg.glow}, 0 0 35px -5px ${cfg.glow}, 0 0 50px -10px rgba(${cfg.rgb}, 0.4)`;
    } else {
      baseShadow += `, 0 0 15px -3px ${cfg.glow}`;
    }
  ...
  return (
    <div
      ...
      style={{
        ...
        background: locked ? "rgba(10,10,15,0.5)" : `linear-gradient(145deg, rgba(${cfg.rgb}, 0.09), rgba(6,6,10,0.88))`,
    ...
    >
      ...
      <div style={{
        ...
        boxShadow: `0 0 12px 3px ${cfg.color}, 0 0 25px ${cfg.glow}`,
      }} />

      {/* Верхний глянцевый блик (Liquid Reflection) */}
      <div className="absolute inset-0 rounded-2xl bg-gradient-to-b from-white/[0.06] to-transparent pointer-events-none z-10 h-1/2 overflow-hidden" />
        ...
        {slot.image_url ? (
          <img
            ...
            style={{
              ...
              animation: (!locked && (owned || isToday)) ? "bp-premium-float 2.2s ease-in-out infinite" : "none",
...
const BattlePass = () => {
  ...
  return (
    <div className="min-h-screen pb-24 relative overflow-x-hidden text-white" style={{
      ...
      {/* Advanced Liquid Glass, Shimmers & Keyframes Engine (Optimized & Accelerated v2026) */}
      <style>{`
        ...
        /* Ускоренная лазерная обводка (в 2 раза быстрее — 3s вместо 6s) */
        ...
        .bp-laser-path {
          ...
          animation: bp-laser-move 3s linear infinite;
        }

        /* Премиальный Hover & Снятие ограничений overflow для свободного свечения теней наружу */
        .bp-card-row { scrollbar-width: none; overflow-y: visible !important; }
        .bp-card-row::-webkit-scrollbar { display: none; }
        
        /* Убран overflow-hidden, чтобы свечение выходило за рамки на 15-30px */
        .bp-card { 
          position: relative;
          transition: all 0.35s cubic-bezier(0.25, 1, 0.5, 1); 
        }
        
        /* Эффектный премиальный Hover: взрывной неоновый Glow наружу и увеличение яркости */
        .bp-card:hover { 
          transform: translateY(-10px) scale(1.03) !important; 
          filter: brightness(1.15);
          border-color: var(--rarity-color) !important;
          box-shadow: 0 25px 45px -5px rgba(0, 0, 0, 0.8), 
                      0 0 30px 2px var(--rarity-glow), 
                      0 0 55px 8px rgba(var(--rarity-rgb), 0.6) !important;
        }
        .bp-card:active { transform: translateY(-3px) scale(0.99) !important; }

        /* Премиальные градиенты карточек */
        .bp-card-premium-legendary {
          background: linear-gradient(145deg, rgba(255,170,0,0.14), rgba(6,5,2,0.94)) !important;
          text-shadow: 0 0 10px rgba(255,170,0,0.5);
        }
        .bp-card-premium-mythic {
          background: linear-gradient(145deg, rgba(255,0,85,0.18), rgba(4,1,5,0.97)) !important;
          text-shadow: 0 0 10px rgba(255,0,85,0.5);
        }

        /* Ускоренные микро-анимации (в 1.5–2 раза быстрее) */
        @keyframes bp-premium-float {
          0%, 100% { transform: translateY(0) scale(1); filter: drop-shadow(0 6px 12px var(--rarity-glow)); }
          50% { transform: translateY(-6px) scale(1.03); filter: drop-shadow(0 14px 24px var(--rarity-glow)); }
        }
        ...
        @keyframes bp-glow-pulse {
          0%, 100% { opacity: 0.85; box-shadow: 0 0 20px var(--btn-glow), inset 0 2px 4px rgba(0,0,0,0.8), inset 0 -2px 4px rgba(255,255,255,0.1); }
          50% { opacity: 1; box-shadow: 0 0 35px var(--btn-glow), inset 0 3px 6px rgba(0,0,0,0.9), inset 0 1px 2px rgba(255,255,255,0.3); }
        }

        /* Кнопка получения награды: Темная плотная подложка, жесткий контраст и объемные внутренние тени */
        .bp-btn-premium {
          position: relative;
          overflow: hidden;
        ...
        /* Ускоренный в 2 раза блик на кнопке (1.8s вместо 3.5s) */
        .bp-btn-premium::after {
          ...
          animation: bp-shimmer-move 1.8s infinite ease-in-out;
        }
        .bp-btn-premium:hover:not(:disabled) {
          filter: brightness(1.2);
          box-shadow: 0 0 30px var(--btn-glow), inset 0 2px 4px rgba(0,0,0,0.8);
        }

        /* Ускоренная в 1.6 раза пульсация активной кнопки кнопки (1.2s вместо 2s) */
        .bp-claim-pulse-active {
          animation: bp-glow-pulse 1.2s infinite ease-in-out;
        }

        /* Специфические фиксы структуры */
          ...
          <div className="px-4 mb-4">
            {canClaim && nextSlot ? (
              <button
                ...
                disabled={claiming}
                className="w-full rounded-2xl py-4 flex items-center justify-center gap-2.5 font-black uppercase tracking-[0.18em] text-xs transition-all bp-btn-premium bp-claim-pulse-active text-white"
                style={{
                  // Добавлена глубокая темная подложка под градиент, усилен контраст текста и заложены выразительные внутренние 3D тени
                  background: `linear-gradient(135deg, rgba(5, 5, 8, 0.85) 0%, rgba(${accentRgb}, 0.35) 50%, rgba(${accentRgb}, 0.1) 100%)`,
                  border: `1px solid rgba(${accentRgb}, 0.7)`,
                  boxShadow: `inset 0 2px 4px rgba(255,255,255,0.25), inset 0 -2px 4px rgba(0,0,0,0.8), 0 0 20px rgba(${accentRgb}, 0.3)`,
                  color: "#ffffff",
                  textShadow: "0 1px 4px rgba(0,0,0,0.8)",
                  "--btn-glow": `rgba(${accentRgb},0.55)`,
                } as any}
              >
                <Gift className="w-4 h-4 animate-bounce" style={{ color: accent }} />
                <span className="drop-shadow-[0_2px_2px_rgba(0,0,0,1)]">{claiming ? "Отримання..." : "Отримати нагороду"}</span>
              </button>
            ) : (
        ...
        <div className="max-w-5xl mx-auto w-full">
          ...
          >
            {pageSlots.map(slot => {
              ...
              return (
                <div key={slot.id} data-day={slot.slot_number} className="py-2">
                  <SlotCard
                    slot={slotWithImg}
  ...
  return (
    <div className="relative w-full rounded-2xl overflow-hidden bp-blur-heavy"
      style={{
        ...
        boxShadow: `0 15px 35px rgba(0,0,0,0.6), 0 0 30px ${cfg.glow}, inset 0 1px 1px rgba(255,255,255,0.15)`,
        ...
        {reward.image_url ? (
          <div className="overflow-hidden rounded-xl bg-black/40 border border-white/5 mb-3.5">
            <img src={reward.image_url} alt={carName}
              ...
              style={{ height:140, animation: "bp-premium-float 2.2s ease-in-out infinite", "--rarity-glow": cfg.glow } as any} />
          </div>
        ) : (
