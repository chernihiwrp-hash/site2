      ...
      <div className="relative" style={{ zIndex: 1 }}>
        ...
        {showSos && (
          <div className="fixed inset-0 z-50 flex items-center justify-center px-4" onClick={() => setShowSos(false)}>
            ...
            <div
              className="relative w-full max-w-sm rounded-2xl p-5 animate-fade-in liquid-glass-strong"
              onClick={e => e.stopPropagation()}
              style={{ border: "1px solid hsl(0 70% 50% / 0.25)", boxShadow: "0 0 40px hsl(0 70% 50% / 0.15)" }}
            >
              <button onClick={() => setShowSos(false)} className="absolute top-3 right-3 text-muted-foreground">
                <X className="w-5 h-5" />
              </button>
              ...
              <div className={`grid gap-2 ${section.items.length === 3 ? "grid-cols-3" : "grid-cols-2"}`}>
                {section.items.map((item, i) => {
                  ...
                  return (
                    ...
                    >
                      <div
                        className={`liquid-glass-card relative overflow-hidden rounded-2xl ${isThreeCol ? "p-3" : "p-4"} flex flex-col gap-2 transition-all duration-200 hover:scale-[1.02] active:scale-[0.97] h-full text-left ${it.red ? "hover:border-destructive/25" : "hover:border-primary/25"}`}
                        style={{
                        ...
                        onMouseEnter={e => {
                          (e.currentTarget as HTMLDivElement).style.boxShadow =
                            `inset 0 1px 0 hsl(0 0% 100% / 0.22), 0 0 24px ${it.red ? "hsl(0 70% 50% / 0.25)" : "hsl(var(--primary) / 0.25)"}`;
                        }}
                        onMouseLeave={e => {
                          (e.currentTarget as HTMLDivElement).style.boxShadow =
                            "inset 0 1px 0 hsl(0 0% 100% / 0.18), inset 0 -1px 0 hsl(0 0% 0% / 0.25), 0 8px 24px hsl(0 0% 0% / 0.35)";
                        }}
                      >
                        {/* Glossy highlight */}
