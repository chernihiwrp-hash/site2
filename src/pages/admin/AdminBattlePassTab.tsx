...
// ─── КОНФІГ РІДКОСТЕЙ ─────────────────────────────────────────────
const RARITY_CONFIG: Record<Rarity, { label: string; color: string; bg: string; border: string; icon: any }> = {
...
// ─── ФОРМА РЕДАГУВАННЯ/СТВОРЕННЯ СЛОТУ ───────────────────────────
const SlotForm = ({
  initial,
  nftGifts,
  onSave,
  ...
  nftGifts: NftGift[];
  onSave: (slot: BattlePassSlot) => void;
  onCancel: () => void;
}) => {
  ...
  return (
    <div
      ...
      style={{ border: `1.5px solid ${cfg.border}`, background: cfg.bg, boxShadow: `0 0 20px ${cfg.color}22` }}
    >
      <div className="flex items-center gap-2 mb-4">
        <Icon className="w-4 h-4" style={{ color: cfg.color }} />
        <span className="text-sm font-bold" style={{ color: cfg.color }}>
          {initial?.id ? "Редагування слоту" : "Новий слот"}
        </span>
      </div>
      ...
      <div className="grid grid-cols-2 gap-3 mb-3">
        ...
        <div>
          ...
          <input
            ...
            onChange={(e) => set("slot_number", parseInt(e.target.value) || 1)}
          />
        </div>
        {/* Назва */}
        <div>
          ...
          <input
            className={inputCls}
            placeholder="Наприклад: Елітна нагорода"
            value={form.title}
            ...
            return (
              <button
                ...
                style={{
                  ...
                  boxShadow: active ? `0 0 12px ${c.color}40` : "none",
                }}
              >
      ...
      <div className="mb-3">
        ...
        <div className="grid grid-cols-2 gap-2">
          {(["cr", "nft", "car", "custom"] as PrizeType[]).map((p) => (
            <button
              key={p}
              onClick={() => set("prize_type", p)}
              className="rounded-xl py-2 px-3 text-[10px] font-bold transition-all text-left"
              style={{
                border: `1px solid ${form.prize_type === p ? cfg.color : "rgba(255,255,255,0.08)"}`,
                background: form.prize_type === p ? cfg.bg : "rgba(255,255,255,0.02)",
                color: form.prize_type === p ? cfg.color : "rgba(255,255,255,0.35)",
              }}
            >
              {PRIZE_LABELS[p]}
            </button>
      ...
      {form.prize_type === "nft" && (
        <div className="mb-3">
          <label className={labelCls}>Оберіть NFT з існуючих</label>
          <select
            className={inputCls}
          ...
          >
            ...
            {nftGifts.map((n) => (
              <option key={n.id} value={n.id}>
                #{n.id} — {n.name || n.title || "NFT"}
              </option>
            ))}
          </select>
          {form.nft_gift_id && (() => {
            const nft = nftGifts.find(n => n.id === form.nft_gift_id);
      ...
      {(form.prize_type === "car" || form.prize_type === "custom") && (
        <div className="mb-3">
          <label className={labelCls}>
            {form.prize_type === "car" ? "Назва машини" : "Назва призу"}
          </label>
          <input
            className={inputCls}
            placeholder={form.prize_type === "car" ? "Наприклад: Ferrari 488" : "Назва вашого призу"}
            value={form.prize_value || ""}
            onChange={(e) => set("prize_value", e.target.value)}
          />
        </div>
      ...
      {/* Зображення (для cr, car, custom) */}
      {form.prize_type !== "nft" && (
        <div className="mb-4">
          <label className={labelCls}>
            URL зображення {form.prize_type === "car" ? "(фото машини)" : "(іконка призу)"}
          </label>
          <input
            className={inputCls}
          ...
          />
          {form.image_url && (
            <div className="mt-2 w-20 h-14 rounded-xl overflow-hidden border border-white/10">
  ...
  return (
    ...
    >
      ...
      <div className="flex-1 min-w-0">
        ...
        <p className="text-[9px] text-white/35 uppercase tracking-wide">
          {cfg.label} • {PRIZE_LABELS[slot.prize_type]}
          {slot.prize_type === "cr" && slot.prize_value ? ` • ${slot.prize_value} CR` : ""}
        </p>
      </div>
    ...
    <div>
      ...
      <div className="flex items-center justify-between mb-5">
        <div className="flex items-center gap-2.5">
          ...
          >
            <Crown className="w-4.5 h-4.5" style={{ color: "#fbbf24", filter: "drop-shadow(0 0 6px rgba(251,191,36,0.8))" }} />
          </div>
          <div>
        ...
        {(["all", "mythic", "legendary", "rare", "common"] as (Rarity | "all")[]).map((r) => {
          ...
          return (
            <button
              ...
              style={{
                border: `1px solid ${active ? (cfg?.color || "rgba(255,255,255,0.5)") : "rgba(255,255,255,0.08)"}`,
                background: active ? (cfg?.bg || "rgba(255,255,255,0.06)") : "rgba(255,255,255,0.02)",
                color: active ? (cfg?.color || "#fff") : "rgba(255,255,255,0.3)",
              }}
            >
              {r === "all" ? "Всі" : cfg?.label}
